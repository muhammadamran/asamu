...
class OLERead
{
    ...
    const client_START_BLOCK_POS = 0x30;
    ...
    public function read($pFilename): void
    {
        ...
        $this->clientStartBlock = self::getInt4d($this->data, self::client_START_BLOCK_POS);
    ...
    public function getStream($stream)
    {
        ...
        if ($this->props[$stream]['size'] < self::SMALL_BLOCK_THRESHOLD) {
            $clientdata = $this->readData($this->props[$this->cliententry]['startBlock']);

            $block = $this->props[$stream]['startBlock'];

            while ($block != -2) {
                $pos = $block * self::SMALL_BLOCK_SIZE;
                $streamData .= substr($clientdata, $pos, self::SMALL_BLOCK_SIZE);
    ...
    private function readPropertySets(): void
    {
        ...
        while ($offset < $entryLen) {
            ...
            // sectorID of first sector of the short-stream container stream, if this entry is client entry
            ...
            } elseif ($upName === 'client ENTRY' || $upName === 'R') {
                // client entry
                $this->cliententry = count($this->props) - 1;
            }
