...
use PhpOffice\PhpSpreadsheet\Shared\OLE\PPS\client;
...
class OLE
{
    ...
    /**
     * client directory of OLE container.
     *
     * @var client
     */
    public $client;
    ...
    public function readPpsWks($blockId)
    {
        ...
        for ($pos = 0; true; $pos += 128) {
            ...
            switch ($type) {
                case self::OLE_PPS_TYPE_client:
                    $pps = new OLE\PPS\client(null, null, []);
                    $this->client = $pps;
            ...
            // check if the PPS tree (starting from client) is complete
            if (isset($this->client) && $this->ppsTreeComplete($this->client->No)) {
                break;
            }
        }
        fclose($fh);
        ...
        foreach ($this->_list as $pps) {
            if ($pps->Type == self::OLE_PPS_TYPE_DIR || $pps->Type == self::OLE_PPS_TYPE_client) {
                $nos = [$pps->DirPps];
                $pps->children = [];
                while ($nos) {
                    $no = array_pop($nos);
     ...
     * starting with the given PPS (not necessarily client).
     *
    ...
    /**
     * Checks whether a PPS is a client PPS or not.
     * If there is no PPS for the index given, it will return false.
     *
     * @param int $index the index for the PPS
     *
     * @return bool true if it's a client PPS, false otherwise
     */
    public function isclient($index)
    {
        if (isset($this->_list[$index])) {
            return $this->_list[$index]->Type == self::OLE_PPS_TYPE_client;
        }
