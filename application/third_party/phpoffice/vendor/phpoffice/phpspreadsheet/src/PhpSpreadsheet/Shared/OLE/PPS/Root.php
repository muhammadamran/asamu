...
class client extends PPS
{
    ...
    {
        parent::__construct(null, OLE::ascToUcs('client Entry'), OLE::OLE_PPS_TYPE_client, null, null, null, $time_1st, $time_2nd, null, $raChild);
    }
    ...
    private function saveBigData($iStBlk, &$raList): void
    {
        ...
        for ($i = 0; $i < $iCount; ++$i) {
            if ($raList[$i]->Type != OLE::OLE_PPS_TYPE_DIR) {
                $raList[$i]->Size = $raList[$i]->getDataLen();
                if (($raList[$i]->Size >= OLE::OLE_DATA_SIZE_SMALL) || (($raList[$i]->Type == OLE::OLE_PPS_TYPE_client) && isset($raList[$i]->_data))) {
